...
Tokenize
"""
#nlp = spacy.load('en_core_web_md') #sm = small, md = medium, lg = large 
nlp = spacy.load('en_core_web_sm') #sm = small, md = medium, lg = large #change to md for final
...
def checkTemplate(srlObjectList): #input: sentenceListForQA[i] or sentenceList[i] 
    ...
    if columnLen == 0: # if there are no SRL results
        ...
        #    qa.append (['Who is ' +  argTags.person + '?','WHO1'])
            ...
            #else:
            #    qa.append(['Why '+ aux + '  ' + prp + ' ' + argTags.vb + ' ' + argTags.nn + '?','WHY3'])
        else:
...
def create_SR(PreprocessedInputFile):
    ...
    for l in dlines:
        if l != '\n': # if not an empty line
            ...
            #print ('Parsing', count_dlines, 'out of', len_dlines) #TODO: Buka lagi nanti di akhir
...
def count_empties(lst, is_outer_list=True):
    if lst == []:
        # the outer list not counted if it's empty
...
###################################### Running the program for trainig ########################################
workPath = os.getcwd()
...
#print('Number of empty questions: ',count_empties(listOfAllGeneratedQA))
